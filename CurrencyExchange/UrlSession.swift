...
class UrlSession: NSObject, URLSessionDelegate, URLSessionTaskDelegate {
    ...
    func getJSON(){
        params = ["": ""]
        let re = NSMutableURLRequest(url: NSURL(string: urlstring)! as URL, cachePolicy: .reloadIgnoringLocalAndRemoteCacheData , timeoutInterval: 15.0)
        do{
            re.httpMethod = "Get"
            print("-----------> [Get] Seting JSON Data <----------")
//            re.httpBody = try JSONSerialization.data(withJSONObject: self.params, options: .prettyPrinted)
            re.addValue("application/json", forHTTPHeaderField: "Content-Type")
//            re.addValue("application/json", forHTTPHeaderField: "Accept")
            
            let session = URLSession.shared
            let task = session.dataTask(with: (re as NSURLRequest) as URLRequest){data,response,error in
                print("-----------> [Session] Get Response <----------")
                if error != nil{
                    print("-----------> [Session] Response Error <----------")
                    self.delegate?.SessionFinishError(error: error! as NSError)
            ...
            print("-----------> [Get] Seting JSON Data Error <----------")
        }
